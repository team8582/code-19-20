...
public class AutoTest extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            /*lift(DRIVE_SPEED, -3, 1);
            robot.liftServo.setPosition(OPEN);

            while(opModeIsActive() && robot.dist.getDistance(DistanceUnit.INCH)>11)
            {
                forwardBackwardDrive(DRIVE_SPEED, -.3, -.33, 1);
            }
            forwardBackwardDrive(DRIVE_SPEED, -.3, -.3, 1);
            lift(DRIVE_SPEED, 3, 1);
            ...
            {
                if(robot.dist.getDistance(DistanceUnit.INCH)<11 && isYellow)
                {
                    sidewaysDrive(DRIVE_SPEED, -.3,-.3, 1); //does this go right or left?
                }
                else if(robot.dist.getDistance(DistanceUnit.INCH)<11 && !isYellow)
                {
                    skystoneCount++;
                    lift(DRIVE_SPEED,-1, 1);
                    robot.liftServo.setPosition(OPEN);
